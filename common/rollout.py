...
class RolloutWorker:
    ...
    def generate_episode(self, episode_counts=None, episode_num=None, agent_obs=None, ask_budget=None, ans_budget=None, evaluate=False, obs_act=None, gmm_manger=None):
        ...
        while not terminated and step < self.episode_limit:
            ...
            if (self.args.alg.find('lsaa') > -1 or self.args.alg.find('adhoc_td') > -1) and (not evaluate):
                for i in range(self.n_agents):
            ...
            for agent_id in range(self.n_agents):
                ...
                if self.args.alg.find('lsaa') > -1 or self.args.alg.find('adhoc_td') > -1:
                    action, ask_budget, probs, ans_budget = self.agents.choose_action(obs[agent_id], last_action[agent_id], agent_id,
                                                       avail_action, epsilon, episode_counts, agent_obs, obs_act, gmm_manger, epi_obs,
                                                                   ask_budget, ans_budget, evaluate=evaluate)
                else:
