...
class Runner:
    ...
    def run(self, num):
        ...
        if self.args.alg.find('lsaa') > -1 or self.args.alg.find('adhoc_td') > -1:
            agent_obs = []
            ...
            for i in range(self.args.n_agents):
                ...
                ans_budget.append(self.args.budget)
        else:
            agent_obs = None
            ans_budget = None
        ...
        while episode_counts < self.args.max_episodes + 1:
            with ((torch.no_grad())):
                ...
                for episode_idx in range(self.args.n_episodes):
                    if self.args.alg.find('lsaa') > -1 or self.args.alg.find('adhoc_td') > -1:
                        ...
                        episode, er, individual_r, steps, _, _ = self.rolloutWorker.generate_episode(episode_counts, episode_idx)
            ...
            if self.args.alg.find('coma') > -1 or self.args.alg.find('central_v') > -1 or self.args.alg.find('reinforce') > -1:
                self.agents.train(episode_batch, train_steps, episode_counts, self.rolloutWorker.epsilon)
                train_steps += 1
            else:
