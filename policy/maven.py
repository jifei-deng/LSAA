...
class MAVEN:
    def __init__(self, args):
        ...
        self.z_policy = HierarchicalPolicy(args)

        self.eval_rnn = BootstrappedRNN(input_shape, args)
        self.target_rnn = BootstrappedRNN(input_shape, args)

        self.eval_qmix_net = QMixNet(args)
        self.target_qmix_net = QMixNet(args)

        self.mi_net = VarDistribution(args)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):
        ...
        class_z = z.long().argmax(dim=-1, keepdim=True)
    ...
    def get_q_values(self, batch, max_episode_len):
        episode_num = batch['o'].shape[0]
        z = batch['z'].repeat(self.n_agents, 1)
        q_evals, q_targets = [], []
    ...
    def save_model(self, train_step, episode_counts):
        ...
        torch.save(self.z_policy.state_dict(), f"{self.model_dir}/{num}({episode_counts})_z_policy_params.pkl")
        torch.save(self.mi_net.state_dict(),  f"{self.model_dir}/{num}({episode_counts})_mi_net_params.pkl")
        torch.save(self.eval_qmix_net.state_dict(), f"{self.model_dir}/{num}({episode_counts})_qmix_net_params.pkl")
