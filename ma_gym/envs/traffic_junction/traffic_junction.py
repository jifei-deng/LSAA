...
class TrafficJunction(gym.Env):
    ...
    def __init__(self, grid_shape=(14, 14), step_cost=-0.01, n_max=4, collision_reward=-10, arrive_prob=0.5,
        ...
        self._agents_direction = [(0, 0) for _ in range(self.n_agents)]
        ...
        self._obs_high = np.ones((mask_size * (self.n_agents + self._n_routes + 2)))
        self._obs_low = np.zeros((mask_size * (self.n_agents + self._n_routes + 2)))
    ...
    def __init_full_obs(self):
        ...
        for agent_i in range(self.n_agents):
            if self.curr_cars_count >= len(self._entry_gates):
                ...
                self._agents_routes[agent_i] = random.randint(1, self._n_routes)
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            _agent_i_obs += [pos[0] / (self._grid_shape[0] - 1), pos[1] / (self._grid_shape[1] - 1)]
    ...
    def step(self, agents_action):
        ...
        if random.uniform(0, 1) < self._arrive_prob:
            ...
            if not all(self._on_the_road) and free_gates:
                ...
                self._agents_routes[agent_to_enter] = random.randint(1, self._n_routes)
