import math
import os
import sys

import numpy as np
import six
from gym import error

if "Apple" in sys.version:
    if "DYLD_FALLBACK_LIBRARY_PATH" in os.environ:
        os.environ["DYLD_FALLBACK_LIBRARY_PATH"] += ":/usr/lib"


try:
    import pyglet
except ImportError as e:
    raise ImportError("Cannot import pyglet. You can install pyglet via 'pip install pyglet'.")

try:
    from pyglet.gl import *
except ImportError as e:
    raise ImportError("Error occured while running `from pyglet.gl import *`. Make sure you have OpenGL installed.")


_BLACK = (0, 0, 0)


def get_display(spec):
    if spec is None:
        return None
    elif isinstance(spec, six.string_types):
        return pyglet.canvas.Display(spec)
    else:
        raise error.Error(
            "Invalid display specification: {}. (Must be a string like :0 or None.)".format(
                spec
            )
        )


class Viewer(object):
    def __init__(self, world_size):
        display = get_display(None)
        self.rows, self.cols = world_size

        self.grid_size = 50

        self.width = self.cols * self.grid_size + 1
        self.height = self.rows * self.grid_size + 1
        self.window = pyglet.window.Window(
            width=self.width, height=self.height, display=display
        )
        self.window.on_close = self.window_closed_by_user
        self.isopen = True

        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)

        script_dir = os.path.dirname(__file__)

        pyglet.resource.path = [os.path.join(script_dir, "icons")]
        pyglet.resource.reindex()

        self.img_box1 = pyglet.resource.image("box_yellow.png")
        self.img_box2 = pyglet.resource.image("box_red.png")
        self.img_coin = pyglet.resource.image("coin.png")
        self.img_treasure = pyglet.resource.image("treasure.png")
        self.img_agent = pyglet.resource.image("agent.png")
        self.img_agent_open = pyglet.resource.image("agent_open.png")
        self.img_agent_pick = pyglet.resource.image("agent_pick.png")

    def close(self):
        self.window.close()

    def window_closed_by_user(self):
        self.isopen = False
        exit()

    def render(self, env, return_rgb_array=False):
        glClearColor(1, 1, 1, 1)
        self.window.clear()
        self.window.switch_to()
        self.window.dispatch_events()

        self._draw_grid()
        self._draw_thing(env)
        self._draw_box(env)
        self._draw_players(env)

        if return_rgb_array:
            buffer = pyglet.image.get_buffer_manager().get_color_buffer()
            image_data = buffer.get_image_data()
            arr = np.frombuffer(image_data.get_data(), dtype=np.uint8)
            arr = arr.reshape(buffer.height, buffer.width, 4)
            arr = arr[::-1, :, 0:3]
        self.window.flip()
        return arr if return_rgb_array else self.isopen

    def _draw_grid(self):
        batch = pyglet.graphics.Batch()
        for r in range(self.rows + 1):
            batch.add(
                2,
                gl.GL_LINES,
                None,
                (
                    "v2f",
                    (
                        0,
                        self.grid_size * r,
                        self.grid_size * self.cols,
                        self.grid_size * r,
                    ),
                ),
                ("c3B", (*_BLACK, *_BLACK)),
            )
        for c in range(self.cols + 1):
            batch.add(
                2,
                gl.GL_LINES,
                None,
                (
                    "v2f",
                    (
                        self.grid_size * c,
                        0,
                        self.grid_size * c,
                        self.grid_size * self.rows,
                    ),
                ),
                ("c3B", (*_BLACK, *_BLACK)),
            )
        batch.draw()

    def _draw_box(self, env):
        idxes = list(zip(*env._box_map.nonzero()))
        ridxes = [env._to_relative_coordinates(pos) for pos in idxes]
        boxes = []
        batch = pyglet.graphics.Batch()

        for row, col in ridxes:
            extend_pos = env._to_extended_coordinates((row, col))
            if env._get_entity_with_pos(extend_pos)[0] == 3:
                boxes.append(
                    pyglet.sprite.Sprite(
                        self.img_box1,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
            elif env._get_entity_with_pos(extend_pos)[0] == 4:
                boxes.append(
                    pyglet.sprite.Sprite(
                        self.img_box2,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
        for b in boxes:
            b.update(scale=self.grid_size / b.width)
        batch.draw()

    def _draw_thing(self, env):
        idxes = list(zip(*env._sth_map.nonzero()))
        ridxes = [env._to_relative_coordinates(pos) for pos in idxes]
        sths = []
        batch = pyglet.graphics.Batch()

        for row, col in ridxes:
            extend_pos = env._to_extended_coordinates((row, col))
            if env._get_entity_with_pos(extend_pos)[0] == 1:
                sths.append(
                    pyglet.sprite.Sprite(
                        self.img_coin,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
            elif env._get_entity_with_pos(extend_pos)[0] == 2:
                sths.append(
                    pyglet.sprite.Sprite(
                        self.img_treasure,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
        for s in sths:
            s.update(scale=self.grid_size / s.width)
        batch.draw()

    def _draw_players(self, env):
        players = []
        batch = pyglet.graphics.Batch()

        for player in env._agents:
            (row, col) = env._to_relative_coordinates(player.pos)
            if player.act == 5:
                players.append(
                    pyglet.sprite.Sprite(
                        self.img_agent_open,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
            elif player.act == 6:
                players.append(
                    pyglet.sprite.Sprite(
                        self.img_agent_pick,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
            else:
                players.append(
                    pyglet.sprite.Sprite(
                        self.img_agent,
                        self.grid_size * col,
                        self.height - self.grid_size * (row + 1),
                        batch=batch,
                    )
                )
        for p in players:
            p.update(scale=self.grid_size / p.width)
        batch.draw()
        for p in env._agents:
            self._draw_badge(*env._to_relative_coordinates(p.pos), p.id, pos_x=3/4, pos_y=1/5)

    def _draw_badge(self, row, col, level, pos_x, pos_y):
        resolution = 6
        radius = self.grid_size / 5

        badge_x = col * self.grid_size + pos_x * self.grid_size
        badge_y = self.height - self.grid_size * (row + 1) + pos_y * self.grid_size

        verts = []
        for i in range(resolution):
            angle = 2 * math.pi * i / resolution
            x = radius * math.cos(angle) + badge_x
            y = radius * math.sin(angle) + badge_y
            verts += [x, y]
        circle = pyglet.graphics.vertex_list(resolution, ("v2f", verts))
        glColor3ub(*_BLACK)
        circle.draw(GL_POLYGON)
        glColor3ub(*_BLACK)
        circle.draw(GL_LINE_LOOP)
        label = pyglet.text.Label(
            str(level),
            bold=True,
            font_name="Times New Roman",
            font_size=12,
            x=badge_x,
            y=badge_y + 1,
            anchor_x="center",
            anchor_y="center",
        )
        label.draw()
