...
class FindTreasure(gym.Env):
        ...
        if len(l) == 0:
            ...
            if box.open:
                if box.something == 2:
                    ...
                    return 1, box
                elif box.type == 2 and box.something == 1:
                    return 2, box
                else:
                    return 0, box
            else:
    ...
    def get_agent_obs(self) -> List[List[float]]:
        ...
        for i, (agent_id, agent) in enumerate(self._agent_generator()):
            ...
            obs[i, 0] = rel_pos[0] / (self._grid_shape[0] - 1)
            obs[i, 1] = rel_pos[1] / (self._grid_shape[1] - 1)
            obs[i, 2] = self._step_count / self._max_steps
            ...
            for pos, j, (_, agent_strength, sth_level, box_level) in zip(mask_pos,
                ...
                entity, box = self._get_entity_with_pos(pos)
                if entity == 0:
                    obs[i, j + 1] = 0
                    obs[i, j + 2] = 0
                elif entity == 1:
    ...
    def step(self, agents_action: List[int]):
        ...
        mask_sth = (np.sum(self._agent_map, axis=2) > 0) & (self._sth_map > 0)
        if np.any(mask_sth):
            pending = self._agent_map[mask_sth]
            poses = np.array(np.where(mask_sth)).transpose()
            for x, pos in zip(pending, poses):
                ag_at_this_grid = np.where(x==1)[0]
                l_ag = list(set(pick_ag).intersection(set(ag_at_this_grid)))
                if len(l_ag) > 0:
        ...
        mask_box = (np.sum(self._agent_map, axis=2) > 0) & (self._box_map > 0)
        if np.any(mask_box):
            pending = self._agent_map[mask_box]
            poses = np.array(np.where(mask_box)).transpose()
            for x, pos in zip(pending, poses):
                ag_at_this_grid = np.where(x == 1)[0]
                if len(ag_at_this_grid) < 2:
                    continue
                l_ag = list(set(open_ag).intersection(set(ag_at_this_grid)))
                if len(l_ag) >= 2:
                    self._box_map[pos[0], pos[1]] = 0
                    entity, box = self._get_entity_with_pos((pos[0], pos[1]))
                    if entity == 0 or entity == 1 or entity == 2:
...
pick_agent_color = (128, 0, 128)
open_agent_color = (0, 128, 128)
