...
class Viewer(object):
    def __init__(self, world_size):
        ...
        self.img_apple = pyglet.resource.image("lu.png")
        self.img_agent = pyglet.resource.image("agent1.png")
    ...
    def _draw_food(self, env):
        idxes = list(zip(*env._tree_map.nonzero()))
        apples = []
        batch = pyglet.graphics.Batch()
        ...
        for row, col in idxes:
            self._draw_badge(row, col, env._tree_map[row, col], pos_x = 5/6, pos_y=1/6)
    ...
    def _draw_players(self, env):
        ...
        for p in env._agents:
            self._draw_badge(*p.pos, p.level, pos_x = 3/6, pos_y=1/4)
